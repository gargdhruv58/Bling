...
public class NotificationActivity extends AppCompatActivity {
    ...
    protected void onStart() {
        ...
        FirebaseRecyclerAdapter<Contacts,NotificationsViewHolder> firebaseRecyclerAdapter
            ...
            @Override
            protected void onBindViewHolder(@NonNull final NotificationsViewHolder holder, int i, @NonNull Contacts model)
            {
                                ...
                                usersRef.child(listUserId).addValueEventListener(new ValueEventListener() {
                                    ...
                                    public void onDataChange(@NonNull DataSnapshot dataSnapshot)
                                    {
                                        if(dataSnapshot.hasChild("image"))
                                        {
                                            ...
                                            Picasso.get().load(imageStr).into(holder.profileIV);
                                        }
                                        final String nameStr=dataSnapshot.child("name").getValue().toString();
                                        holder.userNameTV.setText(nameStr);
                                                                                                        ...
                                                                                                        if(task.isSuccessful())
                                                                                                        {
                                                                                                            friendRequestRef.child(listUserId).child(currentUserId)
                                                                                                                    ...
                                                                                                                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                                                                        @Override
                                                                                                                        public void onComplete(@NonNull Task<Void> task) {
                                                                                                                            Toast.makeText(NotificationActivity.this, "Contact saved successfully", Toast.LENGTH_SHORT).show();
                                                                                                                        }
                                                                                                                    });
                                                                                                        }
                                                                                                    }
    ...
    public static class NotificationsViewHolder extends RecyclerView.ViewHolder
    {
        ...
        public NotificationsViewHolder(@NonNull View itemView) {
            super(itemView);
            userNameTV=itemView.findViewById(R.id.name_notifications);
            acceptBtn=itemView.findViewById(R.id.request_accept_btn);
            cancelBtn=itemView.findViewById(R.id.request_decline_btn);
            profileIV=itemView.findViewById(R.id.image_notifications);
